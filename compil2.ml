...
let rec alloc_expr next env expr =
   ...
   *)
   |TEcall(x ,  l) when (Hashtbl.mem fc ("f"^x)) ->
     let l, fpmax =
      List.fold_left
      (fun (l, fpmax) e ->
   ...
   |TEcall(x, l) ->
      let l, fpmax =
        ...
        (fun (l, fpmax) e ->
          let e, fpmax' = alloc_expr next env e in
          e::l, max fpmax fpmax')([], next) l in
      full_e expr (AElam(Smap.find x env, List.rev l)), fpmax
   ...
   | TEfun    (pl,a,bloc)->
      ...
      let env, fpmax =
        List.fold_right
          (fun x (env, fpmax) ->
            let fpmax = fpmax + 8 in
            Smap.add x (-fpmax) env, fpmax)
          l  (Smap.empty, 8)
      in
      let s = free_var_bloc bloc in
      let t = Hashtbl.create (Sset.cardinal s) in
      Sset.iter (function x ->(
        try (Hashtbl.replace t x (8*((Hashtbl.length t) + 1), Smap.find x env))
        with |Not_found -> print_endline("failed to find "^x); assert false) ) s;
      let ab, nextb = alloc_bloc_lam fpmax env bloc t in
      full_e expr (AEfun(pl,a,ab,nextb,t)), next

and alloc_expr_lam next env expr lib =
  match expr.main with
  |TEcst c ->
    full_e expr (AEcst c), next

  |TEbinop (b,e1,e2) ->
    let ae1, next1 = alloc_expr_lam next env e1 lib in
    let ae2, next2 = alloc_expr_lam next env e2 lib in
    full_e expr (AEbinop(b,ae1,ae2)), max next1 next2

  |TEunop (u, e) ->
    let ae, next = alloc_expr_lam next env e lib in
    full_e expr (AEunop(u, ae)), next

  |TEacc acc ->
    let a,nexta = alloc_acc_lam next env acc lib in
    full_e expr (AEacc(a)),nexta

  | TEeg (acc, e) ->
    let aa, nexta = alloc_acc_lam  next env acc lib in
    let ae, nexte = alloc_expr_lam next env e lib   in
    full_e expr (AEeg(aa,ae)), max nexta nexte
  (*| TEif (e, TB bl, None) ->
    let ae, nexte = alloc_expr_lam next env e lib in
    let ab, nextb = alloc_bloc_lam next env bl lib in
    full_e expr (AEif(ae, AB ab, None)), max nexte nextb*)

  | TEif     (e,blxpr,None)             ->
     let ae ,nexte   = alloc_expr_lam     next env e lib     in
     let ab ,nextb   = alloc_blocexpr_lam next env blxpr lib in
     full_e expr (AEif(ae,ab,None)), max nexte nextb

  (*|TEif(e, TB bl, Some (TB bl2)) ->
    let ae, nexte = alloc_expr_lam next env e lib in
    let ab1, nextb1 = alloc_bloc_lam next env bl lib in
    let ab2, nextb2 = alloc_bloc_lam next env bl2 lib in
    full_e expr (AEif(ae, AB ab1, Some (AB ab2))), max nexte (max nextb1 nextb2)*)

  | TEif     (e,blxpr,Some blxpr2)      ->
     let ae  ,nexte  = alloc_expr_lam     next env e lib      in
     let ab1 ,nextb1 = alloc_blocexpr_lam next env blxpr lib  in
     let ab2 ,nextb2 = alloc_blocexpr_lam next env blxpr2 lib in
     full_e expr (AEif(ae,ab1,Some ab2)), max nexte (max nextb1 nextb2)
  ...
  | TEreturn (Some a)                    ->
     let ae ,nexte = alloc_expr_lam next env a lib  in
     full_e expr (AEreturn (Some ae)), nexte
   ...
   *)
   |TEcall(x ,  l) when (Hashtbl.mem fc ("f"^x)) ->
     let l, fpmax =
      List.fold_left
      (fun (l, fpmax) e ->
         let e, fpmax' = alloc_expr_lam next env e lib in
         e::l, max fpmax fpmax')([], next) l
     in
     full_e expr (AEcall ("f"^x, List.rev l)), fpmax
...
and alloc_acc_lam next env acc lib =
   match acc with
   |TAident (x,l,t) when Hashtbl.mem lib x -> Clovar (fst(Hashtbl.find lib x)), next
   |TAident (x,l,t)   -> (try Locvar (Smap.find x env,l,t),next with |_ -> Glovar(x,l,t),next)
   |TAdot   (a,x,l,t) -> (*Trouver x se fera plus tard apparament*)
                         let aa, nexta = alloc_expr_lam next env a lib in
                         AAdot(aa,x,l,t), nexta
   |TAwhat  (a,x,l,t) ->(*Trouver x se fera plus tard apparament et de voir si c'est null mais bon*)
                         let aa, nexta = alloc_expr_lam next env a lib in
                         AAwhat(aa,x,l,t), nexta
...
and alloc_bloc_lam next env bloc lib =
  let rec traitement next env blc lib =
  match blc with
  | [] -> [], next
  | (TE2 e)::q ->
    let e, nexte = alloc_expr_lam next env e lib in
    let bl, nextb = traitement next env q lib in
      (AE2 e)::bl, max nexte nextb
  ...
  | (TV v)::q ->
    ...
    | TVal (a,b,e,l,t) ->
      ...
      let bl, nextb = traitement n nenv q lib in
      v::bl, max n nextb

  in
  let vxprl,next= traitement next env bloc.blc lib in
  {ablc = vxprl; abtyp = bloc.btyp}, next
...
and alloc_blocexpr_lam next env blxpr lib =
  print_endline("started alloc_blocexpr");
  match blxpr with
  |TB b ->
      print_endline("it's a bloc");
     let ab, nextb = alloc_bloc_lam next env b lib in AB (ab), nextb
  |TE e ->
     let ae, nexte = alloc_expr_lam next env e lib in AE (ae), nexte
...
let lam_code = ref nop
let lam_data = ref nop
let lam_counter   = ref(-1)
let cur_f = ref ""
...
let lam () = string_of_int  !lam_counter
let compare setter =
        movq   (imm 0)   !%r9  ++
        cmpq   !%rbx !%rax ++
        setter !%r9b ++
...
let rec compile_expr e = match e.amain with
 ...
 |AEreturn none ->
  ...
  |AEfun(pl,_,bl,fpmax,t) ->
    ...
    lam_data := !lam_data ++ label ("L"^l) ++ string l;

    let code = ref(
    pushq !%rdi ++
    movq (imm((Hashtbl.length t + 1)*8)) !%rdi ++
    call "malloc" ++
    movq !%rax !%rdi ++
    movq (ilab("L"^l)) (ind rdi)) in
    Hashtbl.iter (fun _ (i,j) -> code := !code ++ movq (ind ~ofs:j rbp) !%rax ++
    movq !%rax (ind ~ofs:i rdi)) t;
    !code ++ popq rdi ++ pushq !%rax

  |AElam(i, l) ->
    let n = (List.length l) + 1 in
    pushq !%r15 ++
    movq (ind ~ofs:i rbp) !%r15 ++
    List.fold_left (++) nop (List.map compile_expr l) ++
    (*pushq (imm i) ++*)
    call_star (ind r15) ++
    popn (n*8)++
